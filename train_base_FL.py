...
if __name__ == '__main__':
    ...
    # ------------------------------
    model_type = 'c3ae_base'
    model_various = 'FL'
    ver = "release"
    save_file_path = ".\\save\\{}_{}_{}\\".format(ver, model_various, model_type)
    ...
    age_count = training_df['age'].value_counts()
    ...
    if use_valid:
        ...
        train_gen = create_data_gen(Xy_train, batch_size=batch_size, mode="all", num_classes=num_classes,
                                    soft_label=soft_label, model_type=model_type, include_gen=include_gen)
    ...
    model.compile(
        optimizer=Adam(learning_rate=learning_rate),
        loss={'cate': focal_loss(age_dict), 'reg': 'mae', "gen": "categorical_crossentropy"},
        metrics={"cate": 'mae', "reg": 'mae', "gen": "accuracy"}
    )
